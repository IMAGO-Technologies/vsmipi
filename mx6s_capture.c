...
static inline void __dma_sync_for_device(struct device *dev, dma_addr_t dma_addr, unsigned long dma_size)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,20,0)
	dev->archdata.dma_coherent = 0;
#else
	dev->dma_coherent = 0;
#endif
	dma_sync_single_for_device(dev, dma_addr, dma_size, DMA_FROM_DEVICE);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,20,0)
...
static inline void __dma_sync_for_cpu(struct device *dev, dma_addr_t dma_addr, unsigned long dma_size)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,20,0)
	dev->archdata.dma_coherent = 0;
#else
	dev->dma_coherent = 0;
#endif
	dma_sync_single_for_cpu(dev, dma_addr, dma_size, DMA_FROM_DEVICE);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,20,0)
	dev->archdata.dma_coherent = 1;
#else
	dev->dma_coherent = 1;
...
static int mx6s_videobuf_prepare(struct vb2_buffer *vb)
{
	...
	__dma_sync_for_device(csi_dev->dev, dma_addr, dma_size);
...
static void mx6s_videobuf_buf_finish(struct vb2_buffer *vb)
{
	...
	__dma_sync_for_cpu(csi_dev->dev, dma_addr, dma_size);
}
...
static int mx6s_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (list_empty(&csi_dev->capture)) {
			struct mx6s_buf_internal *ibuf = list_first_entry(&csi_dev->discard,
						struct mx6s_buf_internal, queue);
			ibuf->bufnum = i;

			if (csi_dev->csi_event_mode)
				__dma_sync_for_device(csi_dev->dev, csi_dev->discard_buffer_dma[i], 4096);
		...
		int bufnum, bool err)
{
	...
	if (list_empty(&csi_dev->capture)) {
		...
		if (csi_dev->csi_event_mode)
			__dma_sync_for_device(csi_dev->dev, csi_dev->discard_buffer_dma[bufnum], 4096);
...
//		dev_dbg(csi_dev->dev, "Start of Frame interrupt.\n");
		...
		list_for_each_entry(ibuf, &csi_dev->active_bufs, queue) {
			...
			__dma_sync_for_cpu(csi_dev->dev, dma_addr, 4096);
				...
				struct v4l2_streamparm *a)
{
	...
	if (a->parm.capture.timeperframe.numerator == 0)
	{
		if (a->parm.capture.timeperframe.denominator == 0) {
			dev_dbg(csi_dev->dev, "stop stream\n");
			v4l2_subdev_call(sd, video, s_stream, 0);
			csi_dmareq_rff_disable(csi_dev);
			csi_enable(csi_dev, 0);
		}
		else {
			dev_dbg(csi_dev->dev, "start stream\n");
			csi_dmareq_rff_enable(csi_dev);
			csi_enable(csi_dev, 1);
			v4l2_subdev_call(sd, video, s_stream, 1);
			csi_error_recovery(csi_dev);
